...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
  ...
  //G4cout << " process is: " << CreatorProcess << G4endl;
  ...
  if (step->GetPostStepPoint()->GetProcessDefinedStep()){
    ...
    if (event_id){
      if (track_id){
	if (name) {
	  //G4cout << "EveTraParXYZdEProc," << event_id << "," << track_id << "," << name << "," << x[0] << "," << x[1] << "," << x[2] << "," << delta_E << "," << procname  << G4endl;
          G4cout << "Event " << event_id << "\tTrack " << track_id << "\tParticle " << name <<  "\t Proc " << procname << "\tdE " << delta_E << "\tX Y Z " << x[0] << "\t" << x[1] << "\t" << x[2] << G4endl; 
	}
      }
    }
  }
